...
namespace AspNetCore3WebApi.Infrastructure.Command
{
  ...
  public sealed class CommandDispatcher : ICommandDispatcher
  {
    ...
    public async Task DispatchCommandAsync<TCommand>(TCommand command) where TCommand : ICommandEntity
    {
      ...
      var handlers = this.serviceProvider.GetServices<ICommandHandlerAsync<TCommand>>();
    ...
    {
      var results = new Collection<Exception>();
      foreach (var condition in this.serviceProvider.GetServices<ICommandPreConditionAsync<TCommand>>())
      {
        try
        {
